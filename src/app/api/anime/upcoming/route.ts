...
export async function GET(request: Request) {
    ...
    try {
        ...
        if (!animeProvider.getUpcoming) {
            return NextResponse.json({ error: "Provider does not support getUpcoming" }, { status: 400 });
        }

        const results: any[] = await animeProvider.getUpcoming();
