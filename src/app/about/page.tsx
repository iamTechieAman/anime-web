...
const stats = [
    ...
    { label: "SEO", value: 100, color: "text-accent" },
];

export default function AboutPage() {
    return (
        <main className="min-h-dvh pt-24 pb-20 px-6 md:px-12 bg-bg-main overflow-x-hidden">
            <div className="w-full max-w-6xl mx-auto space-y-28">
                ...
                <section className="text-center space-y-6">
                    ...
                    >
                        Designed for <br />
                        <span className="bg-clip-text text-transparent bg-gradient-to-r from-accent to-accent-secondary">Cinematic Immersion</span>
                    </motion.h1>
                    ...
                    >
                        <a href="https://github.com/iamTechieAman" target="_blank" rel="noopener noreferrer" className="p-3.5 bg-bg-card rounded-2xl border border-border-color hover:border-accent/55 hover:shadow-[0_0_20px_var(--accent-glow)] transition-all">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                        {stats.map((stat, i) => (
                            <motion.div
                                ...
                                className="p-6 rounded-3xl bg-bg-card/30 border border-border-color backdrop-blur-md text-center flex flex-col items-center justify-center relative group"
                            >
                ...
                <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {[
                        ...
                        <motion.div
                            ...
                            className="p-8 bg-bg-card/40 rounded-3xl border border-border-color relative group hover:border-accent/30 hover:shadow-[0_8px_30px_-8px_var(--accent-glow)] transition-all duration-[250ms] overflow-hidden"
                        >
                ...
                <section className="space-y-12">
                    ...
                    <div className="relative border-l border-border-color ml-4 md:ml-12 space-y-12">
                        {timeline.map((item, i) => (
                            <motion.div 
                                key={item.year}
                            ...
                            >
                                {/* Timeline Dot */}
                                <div className="absolute -left-2.5 top-1.5 w-5 h-5 rounded-full bg-bg-card border-2 border-border-color group-hover:border-accent group-hover:shadow-[0_0_12px_var(--accent-glow)] transition-all duration-[250ms] flex items-center justify-center">
                                    <div className="w-1.5 h-1.5 rounded-full bg-border-color group-hover:bg-accent transition-all" />
                                </div>

                                <div className="space-y-1.5">
                                    <span className="text-xs font-black tracking-widest text-accent uppercase">{item.year}</span>
                ...
                <section className="text-center space-y-10">
                    ...
                    <div className="flex flex-wrap justify-center gap-3 max-w-3xl mx-auto text-xs md:text-sm font-bold">
                        {["Next.js 16", "TypeScript 5", "TailwindCSS v4", "Framer Motion 12", "Python Scrapling", "Edge Runtime", "Lucide Icons", "Axios Cache", "Clerk Authentication"].map((tech) => (
                            <span key={tech} className="px-5 py-3 bg-bg-card/40 border border-border-color rounded-2xl text-[var(--text-muted)] hover:text-white hover:border-accent/55 hover:shadow-[0_4px_20px_-8px_var(--accent-glow)] transition-all cursor-default">
                                {tech}
                            </span>
                        ))}
                    </div>
                ...
                <section className="space-y-12">
                    <div className="bg-bg-card/40 backdrop-blur-md rounded-3xl p-8 md:p-12 border border-border-color relative overflow-hidden group">
                        <div className="absolute top-0 right-0 w-64 h-64 bg-accent/5 blur-[100px] -z-10 group-hover:bg-accent/10 transition-colors duration-[250ms]" />
                        
                        <div className="flex flex-col md:flex-row gap-10 items-center">
                            <div className="w-32 h-32 rounded-full overflow-hidden border-2 border-accent/20 p-1 shrink-0 bg-gradient-to-tr from-bg-card to-bg-main">
                            ...
                            <div className="space-y-4 text-center md:text-left transition-all">
                                <span className="px-3 py-1 bg-accent/10 text-accent text-[10px] font-black uppercase tracking-widest rounded-full border border-accent/20">Lead Developer</span>
                        ...
                        <div className="mt-12 pt-12 border-t border-border-color space-y-6">
