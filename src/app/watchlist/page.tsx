  ...
  return (
    ...
    >
      ...
      <p className="text-sm text-zinc-400 max-w-xs mb-8 leading-relaxed">
        Tap <span className="text-accent font-bold">Add to Watchlist</span> on any title to save it here, organized by folders and tags.
      </p>
  ...
  return (
    <div className="flex items-center gap-1.5 flex-wrap mt-2">
      {tags.map(tag => (
        <span
          key={tag}
          className="inline-flex items-center gap-1 px-2 py-0.5 bg-accent/10 border border-accent/20 text-accent text-[10px] font-bold rounded-lg"
        >
          #{tag}
          <button
      ...
      {isEditing ? (
        <form onSubmit={handleSubmit} className="flex items-center gap-1">
          <input
            ...
            maxLength={24}
            className="w-20 bg-white/5 border border-accent/30 rounded-lg px-2 py-0.5 text-[10px] text-white outline-none focus:border-accent/60 font-bold"
          />
        </form>
      ) : (
        <button
    ...
    >
      <Link href={href} className="absolute inset-0 block w-full h-full select-none">
        ...
        <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
          <div className="w-14 h-14 rounded-full bg-accent flex items-center justify-center scale-75 group-hover:scale-100 transition-transform duration-[250ms] shadow-[0_0_30px_var(--accent-glow)]" will-change-transform>
      ...
      <div className="relative z-10 absolute bottom-0 inset-x-0 p-3 pointer-events-none">
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          {entry.collection && entry.collection !== "All" && (
            <span className="text-[8px] bg-accent/15 text-accent px-1.5 py-0.5 rounded font-bold border border-accent/20">
              {entry.collection}
            </span>
          )}
        </div>
        {entry.tags.length > 0 && (
          <div className="flex items-center gap-1 mt-1 flex-wrap">
            {entry.tags.slice(0, 2).map(tag => (
              <span key={tag} className="text-[8px] text-accent font-bold">#{tag}</span>
            ))}
                ...
                {collections.map(c => (
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 text-[10px] font-bold hover:bg-white/10 transition-colors flex items-center gap-2 ${
                      entry.collection === c ? "text-accent" : "text-zinc-300"
                    }`}
                  >
...
function ListRow({ entry, collections, onRemove, onUpdateCollection, onAddTag, onRemoveTag, href, isDragEnabled }: ListRowProps) {
  ...
  return (
    <Reorder.Item
      ...
      as="div"
      className="group flex items-center gap-3 bg-bg-card border border-border-color hover:border-accent/30 hover:shadow-[0_4px_24px_-8px_var(--accent-glow)] rounded-2xl p-3.5 transition-all duration-[250ms]"
    >
      {/* Drag handle */}
      {isDragEnabled && (
        <div
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        <select
          ...
          onClick={e => e.stopPropagation()}
          className="hidden sm:block bg-bg-elevated border border-border-color rounded-xl text-[10px] text-zinc-300 font-bold px-2.5 py-2 outline-none focus:border-accent/50 cursor-pointer hover:border-white/20 transition-colors"
        >
        ...
        <div className="max-w-[1800px] mx-auto px-4 md:px-10 py-3 space-y-3">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {/* Grid / List toggle */}
              <div className="bg-bg-card border border-border-color p-0.5 rounded-xl flex shrink-0">
                <button
                  onClick={() => { setViewMode("grid"); setIsDragMode(false); }}
                  className={`p-2 rounded-lg transition-all cursor-pointer ${viewMode === "grid" ? "bg-gradient-to-r from-accent to-accent-warm hover:-translate-y-[1px] hover:scale-[1.02] text-white shadow-[0_0_12px_var(--accent-glow)]" : "text-zinc-500 hover:text-white"}`}
                  title="Grid view"
                >
                ...
                <button
                  ...
                  className={`p-2 rounded-lg transition-all cursor-pointer ${viewMode === "list" ? "bg-gradient-to-r from-accent to-accent-warm hover:-translate-y-[1px] hover:scale-[1.02] text-white shadow-[0_0_12px_var(--accent-glow)]" : "text-zinc-500 hover:text-white"}`}
                  title="List view"
                >
          ...
          <AnimatePresence>
            {showAddFolder && (
              ...
              >
                <div className="flex items-center gap-2 flex-1 bg-bg-card border border-accent/30 rounded-xl px-3 py-2 focus-within:border-accent/60 transition-colors">
                ...
                <button type="submit" className="px-3 py-2 bg-gradient-to-r from-accent to-accent-warm hover:-translate-y-[1px] hover:scale-[1.02] hover:bg-accent-secondary text-white text-xs font-black rounded-xl transition-all shrink-0">
                  Create
                </button>
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[11px] font-black uppercase tracking-wider transition-all cursor-pointer border whitespace-nowrap ${
                activeCollection === "All"
                  ...
                  : "bg-bg-card border-border-color text-zinc-400 hover:text-white"
              }`}
            >
        ...
        {watchlist.length === 0 ? (
          ...
          <div>
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="mb-4 flex items-center gap-2 p-3 bg-accent/8 border border-accent/20 rounded-xl"
            >
              <GripVertical className="w-4 h-4 text-accent" />
              <p className="text-xs font-bold text-accent">Drag items to reorder your watchlist</p>
            </motion.div>
            <Reorder.Group
              axis="y"
              values={sorted}
