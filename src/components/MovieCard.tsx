...
export const MovieCard = memo(function MovieCard({ item, type = "movie", isFeatured = false }: { item: MovieItem; type?: string; isFeatured?: boolean }) {
    ...
    return (
        <div
            ref={cardRef}
            className={`group relative w-full card-virtualized transition-all duration-[250ms] ease-out will-change-transform ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-3'} ${isFeatured ? 'col-span-1' : ''}`}
        >
            <Link scroll={false} href={watchHref} className="block w-full focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--accent)] rounded-[18px]" draggable={false}>
                {/* === Poster Container === */}
                <div className="relative w-full overflow-hidden rounded-[18px] bg-[var(--bg-elevated)] shadow-md group-hover:shadow-[var(--shadow-glow-primary)] transition-shadow duration-[250ms]" style={{ aspectRatio: '2/3' }}>
                    ...
                    {(posterSrc && !imgError) ? (
                        <Image
                            ...
                            className="object-cover transition-transform duration-[250ms] ease-out group-hover:scale-[1.02] relative z-[1]"
                            placeholder="blur"
