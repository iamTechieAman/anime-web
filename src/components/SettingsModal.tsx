...
function ToggleSwitch({ value, onChange, color = "bg-accent" }: { value: boolean; onChange: (v: boolean) => void; color?: string }) {
    return (
        <button
            ...
            className={`w-11 h-6 rounded-full p-0.5 transition-colors shrink-0 ${value ? color : 'bg-zinc-700'} cursor-pointer`}
        >
            <motion.div
                className="w-5 h-5 rounded-full bg-white shadow-md animate-none"
        ...
        <ModalPortal isOpen={isOpen} onClose={onClose} className="max-w-4xl">
            <div className="relative p-6 sm:p-8 flex flex-col min-h-0">
                ...
                <div className="flex items-center gap-4 border-b border-white/5 pb-4 mb-6 shrink-0">
                    <div>
                        <h2 className="text-xl md:text-2xl font-black text-white tracking-tight font-sora">App Configuration</h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6 items-start">
                    ...
                    {/* Left Column Tabs */}
                    <div className="flex md:flex-col gap-1 overflow-x-auto md:overflow-x-visible pb-2 md:pb-0 hide-scrollbar bg-white/[0.02] md:bg-transparent p-1.5 md:p-0 rounded-2xl md:rounded-none border border-white/5 md:border-transparent shrink-0">
                        {TABS.map(tab => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`flex items-center gap-3 px-4 py-2.5 text-xs font-black uppercase tracking-wider rounded-xl transition-all cursor-pointer whitespace-nowrap w-full text-left border ${
                                        isActive 
                                            ? 'bg-gradient-to-r from-accent to-accent-warm border-accent/20 text-white shadow-sm' 
                    ...
                    <div className="md:col-span-3 min-w-0 bg-[#0B0713]/30 border border-white/5 rounded-3xl p-4 md:p-6 shadow-xl">
